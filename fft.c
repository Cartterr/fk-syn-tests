...
-					and a bug in cor().
-       Jorge Crempien  06/05/14        add rotation
******************************************************************/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
/* rotate a(t) with angle az */
void rotn(float *r, float *t, float *north, int n, double az) { //JORGE don't know why double works
  int i;
  //printf("n=%i, az=%f and sin=%f\n",n,az,sin(az*PI/180.0)); //JORGE debug
  for(i=0;i<n;i++) {
     north[i] = r[i]*cos(az*PI/180.0)+t[i]*sin(az*PI/180.0);
  }
}

/* rotate a(t) with angle az */
void rote(float *r, float *t, float *east, int n, double az) {
  int i;
  for(i=0;i<n;i++) {
     east[i] = r[i]*sin(az*PI/180.0)-t[i]*cos(az*PI/180.0);
  }
}
