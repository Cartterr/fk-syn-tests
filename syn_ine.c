...
int main(int argc, char **argv) {
  ...
  float dt, dura, rise, rtime, tp, ts, tmp, dist, shift;
  float	*trapezoid(float, float, float, float, int *);
  SACHEAD	hd;
#ifdef SAC_LIB
  ...
  for (i=1; !error && i < argc; i++) {
	...
	switch(argv[i][1]) {
 	   ...
 	   case 'D':
	      j = sscanf(&argv[i][2], "%f/%f/%f",&dura,&rise,&rtime);
	      if (j<2) rise = 0.5;
	      break;
...
earthquake (strike 355, dip 80, rake -70), use:\n\
  ...
  for(i=0; i<nn; i++) {	/* sum up contribution from a DD, DS, SS, and EX */
	...
	if ( i==0 && j== 0 ) {
	   ...
	   if (dura>0.) src = trapezoid(dura,rise,rtime,dt,&ns);
	}
	else if (hd.npts != npt) {
...
float *trapezoid(float dura, float rise, float rtime, float dt, int *ns) {
   int i, nr;
   float t,p,sum;
   float amp, *src;
   *ns = rint((dura+rtime)/dt); if (*ns<2) *ns = 2;
   nr = rint(rtime/dt);
   src = malloc((1+*ns)*sizeof(float));
   if (src == NULL) {*ns=0; return src;}
   p=5*rise/dura;
   sum=0;
   for(i=0;i<nr;i++) src[i]=0.0;
   for(i=nr;i<*ns;i++) 
   {
   t=((i-nr)*dt)/dura;
   src[i]=pow(t,p)*pow(1-t,5-p);
   }
   for(i=0;i<*ns-1;i++) sum=sum+0.5*dt*(src[i]+src[i+1]);
   for(i=0;i<*ns;i++)
   {
   src[i]=src[i]/sum;
   printf("%8.6f\n",src[i]);
