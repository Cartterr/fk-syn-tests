...
int main(int argc, char **argv) {
  ...
  char	nam[128],outnm[128],*ccc,com[3]={'z','n','e'};
  int	polar=0, dynamic=1;
  float	coef,rad[4][3],m0,az,*grn,*syn[3],*n,*e,*src,*pt,disp[3],mt[3][3];
  ...
  for (i=1; !error && i < argc; i++) {
	...
	switch(argv[i][1]) {
 	   ...
 	   case 'A':
	      ...
	      break;
 	   case 'D':
	      j = sscanf(&argv[i][2], "%f/%f/%f",&dura,&rise,&shift);
	      if (j<2) rise = 0.5;
	      break;
  ...
  case 4:
     ...
     //m0 = pow(10.,1.5*m0+16.1-20); // M=log10(Mo)-10.7 => dyne-cm e-20?
     m0 = m0*1.0e-20*1.0e7; //JORGE added to convert from Nm to dyne-cm
     break;
  case 7:
     nn = 4;
     mt_radiat(az,mt,rad);
  ...
  if(argc == 1 || error || (dynamic && src_type == 1 && (*ccc) != 'a') ) {
     fprintf(stderr,"Usage: %s -Mmag([[/Strike/Dip]/Rake]|/Mxx/Mxy/Mxz/Myy/Myz/Mzz) -Aazimuth ([-SsrcFunctionName | -Ddura[/rise/rtime]] [-Ff1/f2[/n]] [-I | -J] -OoutName.z -GFirstCompOfGreen | -P)\n\
   ...
   -D Specify the source time function as a trapezoid,\n\
      give the total duration, rise-time and rupture time (0-0.5, default 0.5=triangle)\n\
...
earthquake (strike 355, dip 80, rake -70), use:\n\
  ...
  //JORGE
  /* rotation of rt to ne */
  if(polar==0){
  n=(float *) calloc(npt, sizeof(float));
  e=(float *) calloc(npt, sizeof(float));
  //printf("%f\n",az); //JORGE debug
 
  rotn(syn[1],syn[2],n,npt,az);
  rote(syn[1],syn[2],e,npt,az);
  syn[1]=n;
  syn[2]=e;  
  }
  //JORGE
  ...
  return 0;
  free(syn);	//JORGE added this to free calloc'ed syn
  if(polar==0){
  free(e);
  free(n);
  }
...
float *trapezoid(float dura, float rise, float dt, int *ns) {
   ...
   *ns = rint((dura)/dt); if (*ns<2) *ns = 2;
   ...
   {
   t=(i*dt)/dura;
   src[i]=pow(t,p)*pow(1-t,5-p);
   }
   for(i=0;i<*ns;i++) sum=sum+0.5*dt*(src[i]+src[i+1]);
   for(i=0;i<=*ns;i++) src[i]=src[i]/sum;
   //for(i=0;i<=*ns;i++) printf("%f %f\n",i*dt,src[i]);
   return src;
   free(src);	//JORGE added this to free src
}
